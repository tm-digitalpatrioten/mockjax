...
(function($) {
 ...
 //mockHandler.proxy != null makes sure this case is only called if we REALLY want to fetch a
            ...
            if(window.env.os === "wp8" && requestSettings.dataType === "json" && mockHandler.proxy != null){
                ...
                var old_success_handler = origSettings.success;

                var success_callback = function(text, status, xhr){
                    text = window.jQuery.parseJSON(text);
                    old_success_handler(text, status, xhr)
                }

                windows_settings.success = success_callback;
                windows_settings.url = url;
                windows_settings.dataType = "text";
                windows_settings.cache = false;
                windows_settings.async = false;
                windows_settings.xhr = function(){
                    return new window.XMLHttpRequestLocal();
                };

                return _ajax.apply($, [windows_settings]);
            }
